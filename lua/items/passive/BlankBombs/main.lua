...
local function IsBlankBomb(bomb)
	...
	bomb = bomb:ToBomb()
	if not BombFlagsAPI.HasCustomBombFlag(bomb, "BLANK_BOMB") then
		return false
	end
...
local function InstaExplode(bomb, player)
...
local function CanBombInstaDetonate(bomb)
	...
	return not (
		wasInRoom
		or bomb.IsFetus
		or bomb.Variant == BombVariant.BOMB_ROCKET
		or bomb.Variant == BombVariant.BOMB_GIGA
		or bomb.Variant == BombVariant.BOMB_ROCKET_GIGA
		or bomb.Variant == BombVariant.BOMB_THROWABLE
		or not BombFlagsAPI.HasCustomBombFlag(bomb, "BLANK_BOMB")
...
local function DoBlankEffect(center, radius)
	...
	local blankExplosion = Isaac.Spawn(
		EntityType.ENTITY_EFFECT,
		RestoredCollection.Enums.Entities.BLANK_EXPLOSION_EFFECT.Variant,
		0,
		center,
		Vector.Zero,
		nil
	...
	for _, projectile in ipairs(Isaac.FindByType(EntityType.ENTITY_PROJECTILE)) do
		...
		if realPosition:DistanceSquared(center) <= (radius * 3) ^ 2 then
			if
				projectile:HasProjectileFlags(ProjectileFlags.ACID_GREEN)
				or projectile:HasProjectileFlags(ProjectileFlags.ACID_RED)
				or projectile:HasProjectileFlags(ProjectileFlags.CREEP_BROWN)
				or projectile:HasProjectileFlags(ProjectileFlags.EXPLODE)
				or projectile:HasProjectileFlags(ProjectileFlags.BURST)
				or projectile:HasProjectileFlags(ProjectileFlags.ACID_GREEN)
			then
...
function BlankBombsMod:OnBombInitLate(bomb)
	...
	if player and not Helpers.GetData(bomb).BombInit then
		...
		local blankChance = player:HasCollectible(RestoredCollection.Enums.CollectibleType.COLLECTIBLE_BLANK_BOMBS)
	...
	if not CanBombInstaDetonate(bomb) then
		return
	end
...
function BlankBombsMod:BombUpdate(bomb)
	...
	if bomb.Variant == BombVariant.BOMB_ROCKET and BombFlagsAPI.HasCustomBombFlag(bomb, "BLANK_BOMB") then
		...
		sprite:ReplaceSpritesheet(
			0,
			"gfx/items/pick ups/bombs/costumes/" .. spritesheetPreffix .. "blank_bombs" .. spritesheetSuffix .. ".png"
		)
		sprite:LoadGraphics()
	end
...
function BlankBombsMod:BlankRocketInit(rocket)
	...
	if
		player:HasCollectible(RestoredCollection.Enums.CollectibleType.COLLECTIBLE_BLANK_BOMBS)
			and rocket:GetDropRNG():RandomInt(100) < 120
...
RestoredCollection:AddCallback(
	...
	BlankBombsMod.OnBlankExplosionUpdate,
	RestoredCollection.Enums.Entities.BLANK_EXPLOSION_EFFECT.Variant
)
...
function BlankBombsMod:OnPlayerDamage(entity, _, _, source)
	...
	if not (bomb:ToBomb() or bomb:ToEffect()) then
		return
	end
	if bomb:ToBomb() and not IsBlankBomb(bomb) then
		return
	end
	if bomb:ToEffect() and not IsBlankEpicFetus(bomb) then
		return
	end
	local bombPlayer = Helpers.GetPlayerFromTear(bomb)
	local player = entity:ToPlayer()

	if not bombPlayer then
		return
	end
...
function BlankBombsMod:OnLocustCollision(locust, collider)
	if locust.SubType ~= RestoredCollection.Enums.CollectibleType.COLLECTIBLE_BLANK_BOMBS then
		return
	end
...
RestoredCollection:AddCallback(
	...
	BlankBombsMod.OnLocustCollision,
...
RestoredCollection:AddCallback(
	...
	{ Item = RestoredCollection.Enums.CollectibleType.COLLECTIBLE_BLANK_BOMBS }
)

---@param position Vector
---@param instant boolean?
---@param spawner Entity?
local function SpawnBlankBomb(position, instant, spawner)
	local bomb = Helpers.SpawnBomb(position, RestoredCollection.Enums.BombVariant.BOMB_BLANK, spawner, 0, {
		"BLANK_BOMB",
	})
	Helpers.GetData(bomb).NancyBlank = not instant
	ChangeSprite(bomb)
	if instant and spawner and spawner:ToPlayer() then
		InstaExplode(bomb, spawner:ToPlayer())
	end
end

---@param bomb EntityBomb
local function MakeBombBlank(bomb)
	BombFlagsAPI.AddCustomBombFlag(bomb, "BLANK_BOMB", RestoredCollection.Enums.BombVariant.BOMB_BLANK)
	ChangeSprite(bomb)
end

RestoredCollection.Bombs.BlankBombs = {
	Spawn = SpawnBlankBomb,
	AddFlagToBomb = MakeBombBlank
}
